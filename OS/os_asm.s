...
SVC_Handler
    ...
    ALIGN
SVC_tableStart
    DCD _svc_OS_enable_irq
    DCD _svc_OS_addTask
    DCD _svc_OS_task_exit
    DCD _svc_OS_yield
    DCD _svc_OS_schedule
	DCD _svc_task_wait
	DCD _svc_task_notify
SVC_tableEnd
...
_task_init_switch
    ...
    ; On entry r0 = &_idleTaskSP (OS_TCB *, actually OS_StackFrame **)
    ; Load r1 = _idleTaskSP (OS_StackFrame *)
    LDR     r1, [r0]
    ; Update PSP
    MSR     PSP, r1
    ; Update _currentTCB
    ...
    ISB
    ; This SVC call should be OS_enableIRQ()
    SVC     0x00
    ; Continue to the idle task
    
    ALIGN
    ; This SVC call should be OS_schedule()
